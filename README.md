...
- [README](#readme)
  ...
  - [使用手順](#使用手順)
    ...
    - [python仮想環境構築](#python仮想環境構築)
      - [venvインストール](#venvインストール)
      - [仮想環境作成](#仮想環境作成)
      - [仮想環境起動](#仮想環境起動)
      - [仮想環境終了](#仮想環境終了)
      - [仮想環境内でパッケージをインストール](#仮想環境内でパッケージをインストール)
  - [メモ](#メモ)
...
- OS: ~~*ubuntu 18.04* on *wsl2* on *windows 10 home*~~ elementary OS 5.1.7 Hera
  - wsl2だと外部に公開するサーバとして運用するには安定しなかったので、Linuxマシンの実機を使う
...
ファイアウォールでクライアントからのパケットをブロックされるとwebサーバへアクセスできないので、ポートを開放する。
ここでは8080番ポートを開放する。
